...
fn part_two(numbers: &Vec<Number>, symbols: &Vec<Symbol>) -> u64 {
    symbols.iter().filter(|s| s.is_gear)
        .filter_map(|s| {
            let (count, gear_ratio) = numbers.iter()
                .filter(|n| n.loc.within_point(&s.loc))
                .map(|s| (1, s.n))
                .fold((0, 1), |(ac, ap), (c, p)| (ac + c, ap * p));
            if count == 2 { Some(gear_ratio) } else { None }
        })
        .sum()
}
